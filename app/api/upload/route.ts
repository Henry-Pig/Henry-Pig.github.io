...
export async function POST(request: Request) {
  try {
    ...
    if (!(file instanceof File)) {
      return NextResponse.json({ success: false, data: null, error: "No image file provided." }, { status: 400 });
    }

    if (!allowedTypes.has(file.type)) {
      return NextResponse.json({ success: false, data: null, error: "Only jpg, png, webp, and gif images are allowed." }, { status: 400 });
    }

    if (file.size > maxSize) {
      return NextResponse.json({ success: false, data: null, error: "Image size must be 5MB or smaller." }, { status: 400 });
    }
